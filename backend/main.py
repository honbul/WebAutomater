...
from recorder import Recorder, Runner, DrissionRunner, DrissionRecorder
...
active_recorder: Optional[Any] = None
...
async def start_recording(request: StartRecordingRequest):
    ...
    try:
        ...
        if request.enable_bypass_mode:
            active_recorder = DrissionRecorder()
            asyncio.create_task(
                asyncio.to_thread(active_recorder.start_recording, request.url)
            )
            mode = "bypass"
        else:
        ...
        return {"status": "started", "url": request.url, "mode": mode}
